...
test("completion popup keeps prompt, status and mouse targets separate", () => {
  ...
  expect(frame.rows[16]).toContain("❯");
  expect(frame.rows[17]).toContain("Next Event:");
  expect(state.layout.actions.filter(hit => hit.action.startsWith("complete:")).every(hit => hit.row < 16)).toBe(true);
  expect(frame.cursor).toContain("17;");
});
